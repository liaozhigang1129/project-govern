...
public class InitiationSowFileController {
    ...
    public ResponseEntity<Resource> download(@PathVariable Long id, @PathVariable Long fileId) {
        InitiationSowFile f = service.list(id).stream()
                ...
                .findFirst()
                .orElseThrow(() -> new com.company.zhiyu.common.exception.BusinessException(404, "SOW file not found: " + fileId));
        Path path = service.resolveFile(f);
        Resource resource = new FileSystemResource(path);
        if (!resource.exists()) {
            throw new com.company.zhiyu.common.exception.BusinessException(404, "File not on disk: " + path);
        }
