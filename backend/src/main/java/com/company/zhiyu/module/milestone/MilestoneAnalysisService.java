...
import com.company.zhiyu.module.milestone.dto.MilestoneAnalysisQuery;
import com.company.zhiyu.module.milestone.dto.MilestoneAnalysisResponse;
import com.company.zhiyu.module.milestone.dto.MilestoneAnalysisResponse.PhaseBucket;
import com.company.zhiyu.module.milestone.dto.MilestoneDrillDownResponse;
...
public class MilestoneAnalysisService {
    ...
    private final com.company.zhiyu.module.project.ProjectRepository projectRepository;
    private final com.company.zhiyu.module.org.UserRepository userRepository;
    private final com.company.zhiyu.module.org.DepartmentRepository departmentRepository;
    ...
    public static LocalDate[] periodRange(String period, LocalDate from, LocalDate to) {
        ...
        if ("custom".equals(period)) {
            if (from == null || to == null) {
                throw new com.company.zhiyu.common.exception.BusinessException(400, "custom 模式必须传 from / to");
            }
        ...
        return switch (period == null ? "this_week" : period) {
            ...
            default -> throw new com.company.zhiyu.common.exception.BusinessException(400, "不支持的 period: " + period);
        };
    }
    ...
    public Set<Long> resolveProjectIds(String scope, Long buId, Long plId, Long viewerUserId) {
        if ("pl".equalsIgnoreCase(scope)) {
            if (plId == null) {
                return projectRepository.findByPmUserIdAndDeletedFalse(viewerUserId).stream()
                        .map(com.company.zhiyu.module.project.Project::getId)
        ...
        if ("bu".equalsIgnoreCase(scope)) {
            if (buId == null) {
                throw new com.company.zhiyu.common.exception.BusinessException(400, "scope=bu 必须传 buId");
            }
    ...
    public MilestoneDrillDownResponse drillDown(MilestoneAnalysisQuery q, Long viewerUserId) {
        ...
        Map<Long, com.company.zhiyu.module.project.Project> projectMap = projectRepository.findAllById(pjIds)
                .stream().collect(Collectors.toMap(com.company.zhiyu.module.project.Project::getId, p -> p));
        Map<Long, com.company.zhiyu.module.org.Department> deptMap = departmentRepository
                .findAllByDeletedFalseOrderByParentIdAscSortOrderAscIdAsc()
                .stream().collect(Collectors.toMap(com.company.zhiyu.module.org.Department::getId, d -> d));
        // 一次性拉所有 PM 用户, 拼 fullName (项目 PM 缺失时给 — 占位)
        Set<Long> pmIds = projectMap.values().stream()
                .map(com.company.zhiyu.module.project.Project::getPmUserId)
                .filter(Objects::nonNull)
                .collect(Collectors.toSet());
        Map<Long, String> userMap = userRepository.findAllById(pmIds).stream()
                .collect(Collectors.toMap(com.company.zhiyu.module.org.AppUser::getId, u -> u.getFullName()));
        ...
        List<MilestoneDrillDownResponse.ProjectRow> rows = matched.stream()
                .map(m -> {
                    com.company.zhiyu.module.project.Project p = projectMap.get(m.getProjectId());
                    if (p == null) return null;
                    com.company.zhiyu.module.org.Department d = p.getDepartmentId() == null ? null : deptMap.get(p.getDepartmentId());
    ...
    private String findRootDeptName(Long deptId, Map<Long, com.company.zhiyu.module.org.Department> deptMap) {
        if (deptId == null) return null;
        com.company.zhiyu.module.org.Department cur = deptMap.get(deptId);
