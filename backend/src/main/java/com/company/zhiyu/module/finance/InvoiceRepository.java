...
public interface InvoiceRepository extends JpaRepository<Invoice, Long> {
    ...
    @Query("""
        ...
        WHERE i.contractId = :cid
          AND i.status IN (com.company.zhiyu.module.finance.Invoice.Status.MATCHED,
                           com.company.zhiyu.module.finance.Invoice.Status.PAID)
          AND i.deleted = false
    """)
    ...
    @Query("""
        ...
        WHERE i.contractId IS NULL
          AND i.deleted = false
          AND i.status = com.company.zhiyu.module.finance.Invoice.Status.PENDING
        ORDER BY i.id DESC
    """)
