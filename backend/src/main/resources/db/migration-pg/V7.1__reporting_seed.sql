...
-- §A 5 个核心数据集 (DS_*)
-- ============================================================
INSERT INTO dataset (code, name, domain, source_table, refresh_policy, status, description)
VALUES
...
-- DS_PROJECT_PROFIT 字段 (10 个)
INSERT INTO dataset_field (dataset_id, field_name, display_name, field_type, data_type, agg_func, dim_role, sort_order)
...
-- §B 8 个角色默认仪表盘 (DASH_*)
-- ============================================================
INSERT INTO dashboard (code, name, scope, is_default, is_shared, refresh_interval_sec, status, description)
VALUES
    ('DASH_PMO_DIRECTOR_HOME', 'PMO 总监首页', 'ROLE', FALSE, TRUE, 300, 'PUBLISHED',
     'PMO_DIRECTOR 角色默认仪表盘 — 7 KPI + 4 图表 + 3 列表(战略全景)'),
    ('DASH_PMO_ADMIN_HOME', 'PMO 管理员首页', 'ROLE', FALSE, TRUE, 300, 'PUBLISHED',
     'PMO_ADMIN 角色默认仪表盘 — 7 KPI + 4 图表 + 3 列表(项目全集)'),
    ('DASH_PMO_HOME', 'PMO 首页', 'ROLE', FALSE, TRUE, 300, 'PUBLISHED',
     'PMO 角色默认仪表盘 — 6 KPI + 3 图表 + 2 列表'),
    ('DASH_EXEC_HOME', '高管首页', 'ROLE', FALSE, TRUE, 600, 'PUBLISHED',
     'EXEC 角色默认仪表盘 — 5 KPI + 3 图表(决策视图)'),
    ('DASH_DEPT_LEAD_HOME', '部门负责人首页', 'ROLE', FALSE, TRUE, 300, 'PUBLISHED',
     'DEPT_LEAD 角色默认仪表盘 — 6 KPI + 3 图表 + 2 列表(部门视角)'),
    ('DASH_PROJECT_LEAD_HOME', '项目负责人首页', 'ROLE', FALSE, TRUE, 180, 'PUBLISHED',
     'PROJECT_LEAD 角色默认仪表盘 — 8 KPI + 4 图表 + 3 列表(操作视图)'),
    ('DASH_TEAM_MEMBER_HOME', '团队成员首页', 'ROLE', FALSE, TRUE, 180, 'PUBLISHED',
     'TEAM_MEMBER 角色默认仪表盘 — 4 KPI + 2 图表 + 1 列表(任务视角)'),
    ('DASH_TASK_USER_HOME', '任务执行人首页', 'ROLE', FALSE, TRUE, 120, 'PUBLISHED',
...
-- §C 9 类报表模板 (RPT_*)
-- ============================================================
INSERT INTO report_template (code, category, name, dataset_id, format, status, description)
VALUES
...
-- config_key 格式:role_dashboard.<ROLE>
-- ============================================================
INSERT INTO system_config (config_key, config_value, value_type, config_group, default_value, description, sort_order, updated_by)
VALUES
