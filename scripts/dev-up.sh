...
start_backend() {
  ...
  setsid bash "$SCRIPT_DIR/dev-backend.sh" "${BACKEND_ARGS[@]}" \
    > /tmp/pmo-backend.stdout 2>&1 &
  BACKEND_PID=$!
  ok "backend pid=$BACKEND_PID, 日志: /tmp/pmo-backend.stdout"
}
...
start_frontend() {
  step "▶ 启动 frontend (后台)"
  cd "$FRONTEND_DIR"
  setsid pnpm dev > /tmp/pmo-frontend.stdout 2>&1 &
  FRONTEND_PID=$!
  ok "frontend pid=$FRONTEND_PID, 日志: /tmp/pmo-frontend.stdout"
}
...
if [[ -n "$BACKEND_PID" ]]; then
  ...
  wait_ready "backend"  "$BACKEND_PID" /tmp/pmo-backend.stdout  "(Started .*Application|Tomcat started on port)" 120 \
    || { cleanup; exit 1; }
fi

if [[ -n "$FRONTEND_PID" ]]; then
  # vite 启动后会有 "Local:   http://localhost:xxxx/"
  wait_ready "frontend" "$FRONTEND_PID" /tmp/pmo-frontend.stdout "Local:.*http://localhost" 60 \
    || { cleanup; exit 1; }
fi
...
if [[ -n "$BACKEND_PID" && -n "$FRONTEND_PID" ]]; then
  tail -n +1 -F /tmp/pmo-backend.stdout /tmp/pmo-frontend.stdout &
  TAIL_PID=$!
elif [[ -n "$BACKEND_PID" ]]; then
  tail -n +1 -F /tmp/pmo-backend.stdout &
  TAIL_PID=$!
else
  tail -n +1 -F /tmp/pmo-frontend.stdout &
  TAIL_PID=$!
fi
