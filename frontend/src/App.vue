...
const menuItems: MenuItem[] = [
  ...
  { path: '/milestones/analysis', label: '里程碑分析', icon: Flag, code: 'MILESTONE_ANALYSIS' },
  {
    ...
    label: 'AI 里程碑预警',
    icon: MagicStick,
    roles: ['PMO_ADMIN', 'ADMIN', 'EXEC', 'DEPT_LEAD', 'PM'],
    code: 'MILESTONE_AI',
  },
    ...
    children: [
      {
        ...
        label: '工时成本核算',
        icon: Histogram,
        roles: ['PMO_ADMIN', 'ADMIN', 'EXEC', 'PM', 'DEPT_LEAD'],
        code: 'COST_USER_MONTH',
      },
      {
        path: '/cost/dashboard',
        label: '多维成本看板',
  ...
  // 商机漏斗 (一级)
  {
    ...
    label: '商机漏斗',
    icon: TrendCharts,
    roles: ['PMO_ADMIN', 'ADMIN', 'EXEC', 'DEPT_LEAD', 'PM', 'SR', 'AR', 'FR'],
    code: 'OPPORTUNITY_FUNNEL',
  },
  // 资源管理 (二级) — 资源管道
  {
    label: '资源管理',
    icon: Box,
    code: 'RESOURCE_MGMT',
    children: [{ path: '/resource-pipeline', label: '资源管道', icon: Box, code: 'RESOURCE_PIPELINE' }],
  },
  {
    label: '风险管理',
    icon: Warning,
...
<template>
  <el-container v-if="auth.token" style="height: 100vh">
    <el-aside width="220px" style="background: #001529">
      <div style="color: white; padding: 16px; font-size: 18px; font-weight: 600">🏗 PMO 治理</div>
      <el-menu
      ...
      >
        <template v-for="i in visibleMenuItems" :key="i.path ?? i.label">
          ...
          <el-sub-menu v-if="i.children?.length" :index="i.path ?? i.label">
            ...
            <el-menu-item v-for="c in i.children" :key="c.path" :index="c.path!">
        ...
        <div style="display: flex; align-items: center; gap: 12px">
          ...
          <el-dropdown @command="(c: string) => c === 'logout' && logout()">
            <span style="cursor: pointer; display: flex; align-items: center; gap: 8px">
              ...
              <span style="color: #909399; font-size: 12px">({{ auth.user?.role }})</span>
            </span>
            <template #dropdown>
              <el-dropdown-menu>
