...
import { Refresh, Search, Edit, Delete, Lock, Plus, Upload, Download, Money } from '@element-plus/icons-vue'
import { costApi, type RoleCostDefaultItem, type HourlyRateItem, type HourlyRateUpsertBody } from '@/api/cost'
...
async function saveRoleDefault() {
  ...
  try {
    ...
    rdDialog.value.submitting = false
  }
}
...
async function loadRates() {
  ...
  try {
    rates.value = await costApi.listHourlyRates(filterUserId.value ? { userId: filterUserId.value } : {})
  } catch (e: any) {
  ...
  <div style="padding: 16px">
      ...
      <el-table v-loading="rdLoading" :data="roleDefaults" border stripe style="width: 100%" empty-text="无">
        ...
        <el-table-column label="角色代码" width="180">
          <template #default="{ row }">
            <code style="background: #f0f4f8; padding: 2px 6px; border-radius: 4px; font-size: 12px">
              {{ row.code }}
            </code>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="120" fixed="right">
          <template #default="{ row }">
            <el-button :icon="Edit" size="small" type="primary" link @click="editRoleDefault(row)">
              编辑
            </el-button>
          </template>
        </el-table-column>
    ...
    <el-card shadow="never">
      ...
      <el-alert type="info" :closable="false" style="margin-bottom: 12px">
        4 级兜底链:
        <b>USER_OVERRIDE &gt; ROLE_OVERRIDE &gt; ROLE_COST_DEFAULT &gt; USER_DEFAULT &gt; 0</b>
    ...
    <el-dialog v-model="rdDialog.visible" :title="'编辑角色档 ' + (rdDialog.row?.code || '')" width="480px">
      <el-form v-if="rdDialog.row" label-width="100px">
        <el-form-item label="角色">
          <code style="background: #f0f4f8; padding: 4px 8px; border-radius: 4px">
            {{ rdDialog.row.code }}
          </code>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="rdDialog.submitting" :icon="Money" @click="saveRoleDefault">
          保存
        </el-button>
      </template>
    </el-dialog>
    ...
    >
      <el-form label-width="100px">
        ...
        <el-form-item label="单人 override">
          <el-input-number
            v-model="dlg.form.userId"
            :min="1"
            controls-position="right"
            placeholder="留空 = 角色档调价"
            style="width: 200px"
            clearable
          />
