...
const filtered = computed(() => {
  ...
  if (kw)
    list = list.filter(
      (i) => i.configKey.toLowerCase().includes(kw) || (i.description || '').toLowerCase().includes(kw),
    )
  return list
})
...
async function reset(item: SystemConfigItem) {
  try {
    await ElMessageBox.confirm(`把 [${item.configKey}] 复位到默认值 [${item.defaultValue}] ?`, '复位', {
      type: 'warning',
    })
...
async function resetAll() {
  ...
  if (dirty.length === 0) {
    ElMessage.info('没有修改过')
    return
  }
  try {
    await ElMessageBox.confirm(`当前有 ${dirty.length} 项被改过,确认全部复位?`, '全部复位', {
      type: 'warning',
    })
...
function typeTag(t: SystemConfigValueType): '' | 'success' | 'warning' | 'info' | 'primary' {
  return ({ STRING: 'info', NUMBER: 'primary', BOOLEAN: 'success', JSON: 'warning', ENUM: 'info' } as const)[
    t
  ]
}
        ...
        <template #default="{ row }">
          ...
          <el-tooltip v-if="isMultiline(row)" :content="row.configValue || '(空)'" placement="top">
            <code style="font-size: 12px">
              {{ (row.configValue || '(空)').slice(0, 60)
              }}{{ row.configValue && row.configValue.length > 60 ? '...' : '' }}
            </code>
          </el-tooltip>
    ...
    <el-form v-if="editDialog.item" label-width="100px">
      <el-form-item label="Key">
        <code style="background: #f0f4f8; padding: 4px 8px; border-radius: 4px">
          {{ editDialog.item.configKey }}
        </code>
      </el-form-item>
      <el-form-item label="类型">
        <el-tag :type="typeTag(editDialog.item.valueType)" size="small">
          {{ editDialog.item.valueType }}
        </el-tag>
      </el-form-item>
      ...
      <el-form-item :label="'当前值 (' + editDialog.item.valueType + ')'">
        ...
        >
          <el-option
            v-for="o in editDialog.item.options.split(',')"
            :key="o.trim()"
            :label="o.trim()"
            :value="o.trim()"
          />
        ...
        <div v-else-if="editDialog.item.valueType === 'JSON'" style="width: 100%">
          <el-input v-model="editDialog.newValue" type="textarea" :rows="6" placeholder='["email"]' />
          <div
            :style="{
              color: editDialog.isJsonError ? '#f56c6c' : '#67c23a',
              fontSize: '12px',
              marginTop: '4px',
            }"
          >
